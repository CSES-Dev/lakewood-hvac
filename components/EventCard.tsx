...
export const EventCard = ({ event }: { event: CommunityEvent }) => {
    ...
    return (
        <Link href={`/community/${event.slug}`}>
            <div className="w-full mx-auto p-6 rounded-2xl hover:shadow-md transition mb-8 bg-[#FFFDF5]">
                <div className="flex items-center space-x-6">


                    {/* Fixed-size date block */}
                    <div className="bg-[#5C6F5F] text-white w-[120px] aspect-square p-2 rounded-lg flex flex-col items-center justify-center">
                        <h1 className="text-lg font-medium">{new Date(event.date).toLocaleString('en-US', { month: 'short' })}</h1>
                        <h1 className="text-4xl font-bold">{new Date(event.date).getDate()}</h1>
